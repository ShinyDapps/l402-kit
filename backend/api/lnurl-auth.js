...
const CHALLENGE_TTL_MS = 5 * 60 * 1000; // 5 min to scan + sign
const TOKEN_TTL_MS = 10 * 60 * 1000; // 10 min deletion token
const DASHBOARD_TOKEN_TTL_MS = 24 * 60 * 60 * 1000; // 24 h dashboard session
const DASHBOARD_SENTINEL = "__dashboard__"; // stored as lightning_address for dashboard challenges
function encodeAsLnurl(url) {
    const words = bech32_1.bech32.toWords(Buffer.from(url, "utf8"));
    return bech32_1.bech32.encode("lnurl", words, 1023).toUpperCase();
}
...
async function handler(req, res) {
    ...
    const { tag, k1: k1Param, sig, key, poll, lightningAddress, dashboard } = req.query;
    ...
    if (!tag && !poll) {
        ...
        const k1 = (0, crypto_1.randomBytes)(32).toString("hex");
    ...
    if (tag === "login") {
        ...
        const lookup = await sbFetch(`/lnurl_challenges?k1=eq.${encodeURIComponent(k1Param)}&select=k1,expires_at,verified,lightning_address&limit=1`);
        ...
        let valid = false;
        try {
            const toU8 = (hex) => Uint8Array.from(Buffer.from(hex, "hex"));
            valid = secp256k1_1.secp256k1.verify(toU8(sig), toU8(k1Param), toU8(key));
        }
        catch {
    ...
    if (poll) {
        const lookup = await sbFetch(`/lnurl_challenges?k1=eq.${encodeURIComponent(poll)}&select=verified,token,token_expires_at&limit=1`);
        const rows = (await lookup.json());
        const row = rows[0];
        if (!row)
            return res.status(404).json({ error: "Challenge not found" });
        if (!row.verified || !row.token)
...
//# sourceMappingURL=lnurl-auth.js.map