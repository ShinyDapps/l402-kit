...
# Uso: .\healthcheck.ps1
# Verifica: testes locais (4 SDKs) + versões publicadas + backend live
...
$tsExit = $LASTEXITCODE { $Matches[1] }
             elseif ($tsOut -match "(\d+) passed")       { $Matches[1] }
             else { "?" }
$tsStatus = if ($tsOut -match "Test Suites:.*\d+ passed" -or $tsOut -match "Tests:.*\d+ passed"s:\s+(\d+) passed")      { $Matches[1] }
...
Pop-Location
goOut -match "^ok" -or $goOut -match "ok\s+") { $PASS } else { $FAIL }
...
$rustStatus = if ($rustOut -match "test result: ok"
...
$pyStatus = if ($LASTEXITCODE -eq 0) { $PASS } else { $FAIL }
...
# /api/invoice — GET sem params: 400, 401, 402 ou 405 (POST-only) sao aceitaveis
try {
