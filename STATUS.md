...
2. ~~**Email Peace**~~ — ✅ **Trocas 4 emails (Peace 2x · nós 2x)**. Alinhamento técnico fechado: Option B confirmada por ambos. Última resposta enviada 2026-05-27 com update 1.10.1 cross-lang + NWCWallet + oferta fixture CloudEvents. Aguardando: (a) Peace aceitar fixture → gero JSON em 15min, OU (b) Peace pingar quando staging ingest stable → ativa Option A. **NÃO COBRAR.**
...
- 🔵 [Glama MCP Connectors submission](https://glama.ai/mcp/connectors) — **SUBMITTED 2026-05-27** via aba Connector. Nome `l402-kit-mcp`, URL `https://l402kit.com/api/mcp`, Private Notes explicando 402 protocol (não é failure). Domain claim via `/.well-known/glama.json` (deployed `ffe69a5`). Aguardando review humana 1-3 dias. Frank já nos conhece do PR #5585.
