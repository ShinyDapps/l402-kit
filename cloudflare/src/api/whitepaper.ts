...
<h2>Preface</h2>
<p>I built l402-kit because I kept running into the same wall. I had data worth selling — per query, not per month — and no infrastructure that could handle a transaction worth a fraction of a cent without collapsing under its own overhead. Stripe's minimum charge is $0.50 after fees. PayPal requires accounts on both sides. Subscriptions bundle demand that isn't bundled. None of these are bugs in those systems. They are consequences of being built for a specific kind of commerce — human, high-latency, high-trust — that is not the commerce I was trying to do.</p>
<p>What I was trying to do was closer to what electricity meters do: measure consumption precisely, charge exactly for what was used, settle immediately, require no relationship between the meter and the appliance. The Lightning Network is the closest thing that exists to this model in software. The L402 protocol wraps it in HTTP. The gap between the protocol and a usable middleware library was the thing worth building.</p>
<p>The bet this project makes is not a bet on Bitcoin's price. It is a bet on a simpler and longer-lived claim: that programmatic commerce between machines will grow, that the tools for that commerce will need to be as programmable as the machines themselves, and that a payment mechanism requiring human intervention at the authentication step is not a payment mechanism suited for autonomous systems. Agents that cannot pay cannot be fully autonomous. APIs that cannot charge cannot sustain themselves on something other than advertising or venture capital. The missing primitive is not exotic — it is just HTTP 402, finally implemented.</p>
