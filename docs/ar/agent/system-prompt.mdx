---
title: "مقتطف موجه النظام"
description: "أضف l402-kit إلى موجه النظام الخاص بوكيلك حتى يدفع مقابل واجهات برمجة التطبيقات تلقائيًا."
---

## مقتطف للنسخ واللصق

أضف هذا إلى موجه النظام لأي وكيل لتمكين مدفوعات L402 المستقلة:
...
استبدل `{BUDGET_SATS}` بميزانية الجلسة الخاصة بك (مثلًا `1000` لما يعادل ~$0.50).
...
- اضبط `budgetSats` بشكل متحفظ — يمكن للوكلاء تراكم الاستدعاءات بسرعة
- استخدم `spendingReport()` لتسجيل الاستخدام في نهاية الجلسة
- للبيئات الإنتاجية، استخدم [BudgetManager](/agent/budget) مع حدود لكل مستخدم