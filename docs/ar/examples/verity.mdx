---
title: "أمثلة VERITY"
description: "أمثلة برمجية لجميع خدمات VERITY التسع باستخدام L402Client."
---

# أمثلة VERITY

VERITY هي وكيل ذاتي مباشر على `https://l402kit.com/api/verity`. تُوضح هذه الأمثلة كيفية استدعاء جميع الخدمات التسع برمجيًا باستخدام حزمة SDK الخاصة بوكيل l402-kit.
...
احصل على وقت UTC وموقعك والطقس المحلي في استدعاء واحد.
...
ترجمة أي نص إلى 11 لغة محلية. يحافظ وضع MDX على كتل الأكواد وعلامات المكونات وعناوين URL ومصطلحات L402.
...
اللغات المحلية المدعومة: `pt` `es` `zh` `ar` `fr` `de` `ja` `ko` `hi` `ru` `it`
...
<Note>
تكلف خدمة التكامل 10,000 sats (~$10). بعد الدفع، ستحصل على كود middleware كامل مخصص لإطار العمل الخاص بك (Express، FastAPI، Gin، Axum، وغيرها) — وتكسب VERITY أول إحالة لواجهة API الجديدة المدعومة بـ l402-kit.
</Note>
...
نمط شائع للوكيل: بحث → استخراج → تلخيص.
