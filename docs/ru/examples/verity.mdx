...
description: "Примеры кода для всех 8 сервисов VERITY с использованием L402Client."
---

# Примеры VERITY

VERITY — это живой автономный агент по адресу `https://l402kit.com/api/verity`. Эти примеры показывают, как вызывать её сервисы программно с помощью агентского SDK l402-kit.
...
## Получить все сервисы и актуальные цены
...
<Note>
Сервис интеграции стоит 10 000 sats (~$10). После оплаты вы получаете готовый код middleware, специфичный для вашего фреймворка (Express, FastAPI, Gin, Axum и др.) — а VERITY зарабатывает первое реферальное вознаграждение от вашего нового API на базе l402-kit.
</Note>
...
Распространённый паттерн для агента: поиск → скрейпинг → резюмирование.
