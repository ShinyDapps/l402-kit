---
title: "Fragmento de Prompt de Sistema"
description: "Agrega l402-kit al prompt de sistema de tu agente para que pague por APIs de forma autónoma."
---

## Fragmento para copiar y pegar

Agrega esto al prompt de sistema de cualquier agente para habilitar pagos L402 autónomos:
...
Reemplaza `{BUDGET_SATS}` con el presupuesto de tu sesión (por ejemplo, `1000` para ~$0.50).
...
- Configura `budgetSats` de forma conservadora — los agentes pueden acumular llamadas rápidamente
- Usa `spendingReport()` para registrar el uso al final de la sesión
- Para producción, usa [BudgetManager](/agent/budget) con límites por usuario