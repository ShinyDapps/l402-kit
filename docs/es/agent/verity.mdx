---
title: "VERITY"
description: "Agente de IA autónomo con 9 servicios protegidos por L402. Gana sats, paga en sats."
---

# VERITY

VERITY es un agente de IA autónomo construido sobre l402-kit. Vende 9 servicios a través del protocolo L402, acumula un tesoro de Bitcoin y ajusta sus propios precios según la demanda — sin intervención humana.
...
| Estado Mundial | `/api/verity/worldstate` | 80 sats | — |
...
# Paso 2 — pagar la factura con cualquier wallet de Lightning
# Paso 3 — reintentar con la prueba de pago
...
### Búsqueda — 100 sats
Búsqueda web que devuelve los 10 principales resultados orgánicos.
...
WHOIS, registros DNS y certificados SSL. Sin clave de API — utiliza RDAP público y crt.sh.
...
### Traducir — 50 sats
Traduce texto o documentación MDX a cualquiera de los 11 idiomas soportados. Cuando el `format` es `mdx`, los bloques de código, componentes MDX, URLs y términos técnicos se conservan exactamente — solo se traduce el texto en prosa.
...
**Formato MDX** (conserva bloques de código y componentes):
```bash
curl -X POST -H "Authorization: L402 <token>" \
  -H "Content-Type: application/json" \
...
Si haces un fork de VERITY para ejecutar tu propio agente, estos endpoints te permiten cambiar precios y presupuestos en tiempo de ejecución — sin necesidad de un nuevo despliegue.
...
Actualiza los parámetros de precios para un servicio específico. Solo se modifican los campos proporcionados.
...
| `cogs` | Costo de los bienes en sats (usado por el agente fiscal) |
...
Mínimo: 100 sats. Los cambios tienen efecto de forma inmediata.
...
VERITY (agent:shinydapps.verity)
  ├── 9 servicios protegidos por L402
  ├── Tesoro: shinydapps@blink.sv
  ├── Latido: cada 30 minutos (ajuste de precios)
  ├── Agente Fiscal: informe diario (sats ganados + equivalente en BRL)
