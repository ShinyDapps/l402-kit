---
title: "Extrait de Prompt Système"
description: "Ajoutez l402-kit au prompt système de votre agent pour qu'il paye les APIs de manière autonome."
---

## Extrait à copier-coller

Ajoutez ceci au prompt système de n'importe quel agent pour activer les paiements L402 autonomes :
...
Remplacez `{BUDGET_SATS}` par votre budget de session (par ex. `1000` pour ~0,50 $).
...
- Définissez `budgetSats` de manière conservatrice — les agents peuvent accumuler des appels rapidement
- Utilisez `spendingReport()` pour journaliser l'utilisation en fin de session
- Pour la production, utilisez [BudgetManager](/agent/budget) avec des limites par utilisateur