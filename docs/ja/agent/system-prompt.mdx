---
title: "システムプロンプトスニペット"
description: "エージェントのシステムプロンプトにl402-kitを追加して、APIへの支払いを自律的に行えるようにします。"
---

## コピー＆ペーストスニペット

自律的なL402支払いを有効にするには、任意のエージェントのシステムプロンプトに以下を追加してください：
...
`{BUDGET_SATS}` をセッションの予算に置き換えてください（例：~$0.50の場合は`1000`）。
...
- `budgetSats` は控えめに設定してください — エージェントはAPIコールを急速に積み重ねることがあります
- セッション終了時に使用状況を記録するには `spendingReport()` を使用してください
- 本番環境では、ユーザーごとの上限を設定できる [BudgetManager](/agent/budget) を使用してください