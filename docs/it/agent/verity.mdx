...
VERITY è un agente AI autonomo costruito su l402-kit. Vende 9 servizi tramite il protocollo L402, accumula un treasury in Bitcoin e adegua i propri prezzi in base alla domanda — senza intervento umano.
...
I prezzi sono dinamici — VERITY li adegua ogni 30 minuti in base alla domanda.
...
Restituisce il contenuto completo della pagina in markdown.
...
Invia l'URL pubblico di un repository GitHub. VERITY analizza il codice sorgente e restituisce il codice di integrazione l402-kit completo.
...
### Stato del Mondo — 80 sats
Orario UTC + geolocalizzazione del chiamante + meteo locale in una singola chiamata. Nessun costo per API esterne.
...
Lunghezza massima dell'input: 100.000 caratteri per chiamata.
...
Protezione minima                    → il prezzo non scende mai sotto il minimo
```
...
Se effettui un fork di VERITY per eseguire il tuo agente, questi endpoint ti consentono di modificare prezzi e budget a runtime — senza bisogno di un nuovo deploy.

Tutte le route di amministrazione richiedono l'intestazione `DASHBOARD_SECRET`:
```
Authorization: Bearer <DASHBOARD_SECRET>
```

### GET /api/verity/admin/config
Restituisce la configurazione dei prezzi corrente e la spesa dei consumer per tutti i servizi.
...
| `floor` | Prezzo minimo (il pricing dinamico non scende mai al di sotto) |
| `surgeThreshold` | Chiamate/ora che attivano un aumento del +10% |
| `cogs` | Costo della merce in sats (utilizzato dall'agente fiscale) |

### POST /api/verity/admin/budget
Imposta il limite di spesa giornaliero del consumer.
...
VERITY (agent:shinydapps.verity)
  ...
  ├── Heartbeat: ogni 30 minuti (adeguamento prezzi)
