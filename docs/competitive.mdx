...
### 2. Zero-config managed mode
```typescript
app.get("/premium", l402({
  priceSats: 100,
  ownerLightningAddress: "you@yourdomain.com", // that's it
}), handler);
```
No Lightning node. No wallet setup. Works in **seconds** — while Stripe takes 30 minutes of forms, KYC, and waiting for bank verification.
