...
interface L402Options {
  priceSats: number;
  lightning: LightningProvider;        // required since v1.4.0
  supabaseUrl?: string;
  supabaseKey?: string;
  ...
  /** @deprecated use lightning: ManagedProvider.fromAddress(addr) */
...
### v1.3.x → v1.4.0 (breaking)

`lightning` is now required. `ownerLightningAddress` is deprecated.

```typescript
// Before (v1.3.x)
l402({ priceSats: 10, ownerLightningAddress: 'you@blink.sv' })

// After (v1.4.0) — sovereign
import { AlbyProvider } from 'l402-kit';
l402({ priceSats: 10, lightning: new AlbyProvider(process.env.ALBY_TOKEN!) })

// After (v1.4.0) — cloud managed (explicit opt-in)
import { ManagedProvider } from 'l402-kit';
