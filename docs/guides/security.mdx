...
```
1. Client calls GET /api/lnurl-auth → server returns {k1, lnurl}
2. User scans the LNURL-auth QR with their wallet
3. Wallet signs k1 with the secp256k1 key derived for this domain
4. Wallet calls GET /api/lnurl-auth?k1=...&sig=...&key=...
5. Server verifies the signature → issues a short-lived deletion token
6. Client sends POST /api/delete-data with {lightningAddress, token}
7. Server validates token → deletes data → revokes token
```
